...
class PhotoController extends FrontController
{
	...
	public function getForm($postIdOrToken)
	{
		...
		if (config('settings.single.publication_form_type') == '2') {
			$postEditionUrl = lurl('edit/' . $postIdOrToken);
		...
		if (getSegment(2) == 'create') {
			if (!session()->has('tmpPostId')) {
				$postCreationUrl = lurl('posts/create');
		...
		if (
			...
			$nextStepUrl = $localePath . 'posts/' . $creationPath . $postIdOrToken . '/payment';
			$nextStepLabel = t('Next');
		} else {
			if (getSegment(2) == 'create') {
				$nextStepUrl = $localePath . 'posts/create/' . $postIdOrToken . '/finish';
			} else {
